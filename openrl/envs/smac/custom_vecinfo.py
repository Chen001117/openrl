        ...
        for step_info in self.infos:
            for singe_env_info in step_info:
                ...
                if "final_info" in singe_env_info.keys():
                    try:
                        assert (
                            "battle_won" in singe_env_info["final_info"].keys()
                        ), "battle_won must be in info"
                        assert singe_env_info["final_info"]["battle_won"] in [
                            False,
                            True,
                        ], "battle_won in the final_info must be win or lose"
                        self.win_history.append(
                            singe_env_info["final_info"]["battle_won"]
                        )
                    except:
                        print("battle_won must be in info!")
                        print("final_info")
                        print(singe_env_info["final_info"])
