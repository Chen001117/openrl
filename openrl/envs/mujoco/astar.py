...
def heuristic_cost_estimate_fnct(current: T, goal: T) -> float:
    current = np.array([current//500, current%500])
    goal = np.array([goal//500, goal%500])
    return np.linalg.norm(current-goal)

def distance_between_fnct(n1: T, n2: T) -> float:
    n1 = np.array([n1//500, n1%500])
    n2 = np.array([n2//500, n2%500])
...
def is_goal_reached_fnct(current: T, goal: T) -> bool:
    current = np.array([current//500, current%500])
    goal = np.array([goal//500, goal%500])
    return np.linalg.norm(current-goal) < 5 * np.sqrt(2)
...
def find_path(
    ...
    goal = int(goal[0]) * 500 + int(goal[1])
    ...
    class FindPath(AStar):
        ...
        def neighbors(self, node) -> Iterable[U]:
            return neighbors_fnct(node, obs_map)  # type: ignore
