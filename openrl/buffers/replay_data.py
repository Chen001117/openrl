...
class ReplayData(object):
    def __init__(
        ...
        self.value_preds = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, self.num_data_aug, 1),
            dtype=np.float32,
        )
        ...
        self.actions = np.zeros(
            (self.episode_length, self.n_rollout_threads, self.num_data_aug, act_shape),
            dtype=np.float32,
        )
        ...
        self.rewards = np.zeros(
            (self.episode_length, self.n_rollout_threads, self.num_data_aug, 1),
            dtype=np.float32,
        )
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        for indices in sampler:
            ...
            for index in indices:
                ind = index * data_chunk_length
                batch_per_env = self.episode_length * self.num_agents
                ind = ((ind//batch_per_env) * (batch_per_env*2)) + (ind % batch_per_env)
                # size [T+1 N M Dim]-->[T N M Dim]-->[N,M,T,Dim]-->[N*M*T,Dim]-->[L,Dim]
                if self._mixed_obs:
                    for key in critic_obs.keys():
